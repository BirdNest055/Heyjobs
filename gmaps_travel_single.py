#!/usr/bin/env python3
"""
Google Maps Travel Scraper - Single Town Runner
Processes one town at a time. Call repeatedly to continue.
"""
...
def search_and_extract(page, town, search_term, max_scroll=10):
    companies = []
    seen_names = set()
    ...
    for idx, (item, href) in enumerate(unique_items):
        try:
            ...
            if detail.get("name"):
                ...
                w = detail.get('website', '-')[:35]
                p = detail.get('plz', '-')
                c = detail.get('category', '-')[:20]
                print(f"    [{idx+1}/{len(unique_items)}] {detail['name'][:35]} | web={w} | plz={p} | cat={c}", flush=True)
            elif name:
                detail["name"] = name
                companies.append(detail)
...
def main():
    print("=== GMaps Travel Scraper (single-town mode) ===", flush=True)
    ...
    # Load data
    progress = load_json(PROGRESS_FILE, {"completed_searches": [], "phase": "search_new_towns"})
    ...
    print(f"Missing towns ({len(missing)}): {missing[:5]}...", flush=True)

    if not missing:
        print("All towns searched! Moving to enrichment phase.", flush=True)
    ...
    with sync_playwright() as p:
        ...
        # Phase 1: Search missing towns (one at a time)
        if progress.get("phase") == "search_new_towns":
            for town_name in missing:
                ...
                for term in terms:
                    ...
                    try:
                        ...
                        print(f"  [ERR] {town_name} {term}: {e}", flush=True)
                        try:
                    ...
                    print(f"  New: {new_count} | Total: {len(all_companies)} | Push: {push_counter}", flush=True)
                    ...
                    if push_counter >= 10:
                        ...
                        git_push(f"GMaps: {town_name} +{push_counter} | {len(all_companies)} total")
                        push_counter = 0

                    time.sleep(random.uniform(1, 2))

                print(f"  Town {town_name} done. Total: {len(all_companies)}", flush=True)

            progress["phase"] = "enrich_existing"
            save_json(PROGRESS_FILE, progress)
            create_excel(list(all_companies.values()))
            git_push(f"GMaps Phase1 DONE | {len(all_companies)} total")

        # Phase 2: Enrich existing companies
        if progress.get("phase") == "enrich_existing":
            print(f"\n=== PHASE 2: Enriching ===", flush=True)
            ...
            print(f"Enrichment: {len(need)} (P1 web:{p1} P2 plz:{p2})", flush=True)
                ...
                try:
                    ...
                    if first:
                        ...
                        if detail:
                            ...
                            if comp.get("website"):
                                print(f"  [E] {name[:30]}: web={comp['website'][:35]}", flush=True)
                except:
                    pass
    ...
    print(f"\n=== Run complete | {len(all_companies)} companies ===", flush=True)
