#!/usr/bin/env python3
"""Google Maps Headless Scraper - No Xvfb needed."""
...
def make_browser():
    ...
    b = p.chromium.launch(headless=True, args=['--no-sandbox','--disable-dev-shm-usage','--disable-blink-features=AutomationControlled','--disable-gpu'])
    ctx = b.new_context(viewport={'width':1920,'height':1080}, user_agent='Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/121.0.0.0 Safari/537.36', locale='de-DE')
    ctx.add_init_script('Object.defineProperty(navigator, "webdriver", {get: () => undefined}); window.chrome = {runtime: {}};')
    page = ctx.new_page()
    return p, b, ctx, page

def close_browser(p, b):
    try:
        b.close()
    except:
        pass
...
def main():
    max_towns = int(sys.argv[1]) if len(sys.argv) > 1 else 999
                ...
                try:
                    ...
                    raw = page.evaluate(r"""() => {
                        ...
                        document.querySelectorAll('a[href*="/maps/place/"]').forEach(l => {
                            try {
                                ...
                                const t = c.innerText || '';
                                const lines = t.split('\n').filter(l => l.trim());
                                b.push({name:a.substring(0,150), href:h, text:t.substring(0,800), lines:lines.slice(0,12)});
                            } catch(e) {}
                        });
                        return b;
                    }""")
                    ...
                    for r in (raw or []):
                        ...
                        if k and len(k) > 2 and k not in seen:
                            ...
                            category = ''
                            cat_p = r'(IT-Berater|Softwareentwicklung|Restaurant|Café|Bäckerei|Friseur|Apotheke|Steuerberater|Rechtsanwalt|Versicherung|Immobilien|Bauunternehmen|Elektroinstallateur|Maschinenbau|Metallbau|Kfz-Werkstatt|Autohaus|Logistik|Marketing|Werbeagentur|Handwerker|Schreiner|Maler|Dachdecker|Dienstleistung|Gastronomie|Hotel|Fitness|Gesundheit|Zahnarzt|Bank|Architekt|Ingenieurbüro|Gartenbau|Florist|Optiker|Fahrschule)'
                            for line in lines[:5]:
                                m = re.search(cat_p, line, re.I)
                                if m:
                                    category = m.group(1)
                                    break
                            ...
                            all_results.append({
                                'name':n, 'rating':rating, 'category':category, 'address':address,
                                'plz':plz, 'city':city, 'phone':phone, 'has_website':has_website,
                                'lat':lat, 'lon':lon, 'href':href, 'search_town':name,
