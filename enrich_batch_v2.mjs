...
// Query SQLite using a helper script
function query(sql) {
    const fs = require('fs');
    const tmpFile = '/tmp/db_query.py';
    fs.writeFileSync(tmpFile, `
import sqlite3, json, sys
conn = sqlite3.connect('${DB}')
rows = [list(r) for r in conn.execute(${JSON.stringify(sql)}).fetchall()]
print(json.dumps(rows))
conn.close()
`);
    const result = execSync(`python3 ${tmpFile}`, {encoding:'utf8'});
    return JSON.parse(result);
}

function run(sql, params=[]) {
    const fs = require('fs');
    const tmpFile = '/tmp/db_run.py';
    fs.writeFileSync(tmpFile, `
import sqlite3, json
conn = sqlite3.connect('${DB}')
...
console.log('=== Batch SDK Enrichment v2 ===\n');
