#!/usr/bin/env python3
"""
Continue Google Maps scraping for remaining towns around Erlangen.
Picks up where previous run left off using progress tracking.
"""
...
LOG_FILE = '/home/z/my-project/gmaps_continue_log.txt'
...
def search_google_maps(page, query, max_scrolls=SCROLL_ITERATIONS):
    ...
    if "sorry" in page.url or "captcha" in page.url.lower():
        log("  ⚠️ CAPTCHA/Block detected! Waiting 60s...")
        time.sleep(60)
        return []
...
def main():
    log("=" * 60)
    log("Google Maps CONTINUE - Remaining Towns")
    log("=" * 60)
    ...
    progress = {'completed_searches': [], 'last_town_idx': 0, 'last_term_idx': 0}
    if os.path.exists(PROGRESS_FILE):
        with open(PROGRESS_FILE, 'r') as f:
            progress = json.load(f)
    
    # Find remaining towns
    completed_town_names = set()
    for cs in progress['completed_searches']:
        town = cs.split('|')[0]
        completed_town_names.add(town)
    
    remaining_towns = [t for t in all_towns if t['name'] not in completed_town_names]
    remaining_towns.sort(key=lambda x: x['distance_km'])
    
    log(f"All towns: {len(all_towns)}, Completed: {len(completed_town_names)}, Remaining: {len(remaining_towns)}")
    log(f"Existing results: {len(all_results)}")
    
    if not remaining_towns:
        log("No remaining towns! All done.")
        return
    
    for t in remaining_towns[:5]:
        log(f"  -> {t['name']} ({t['distance_km']}km, tier={t['population_tier']})")
    if len(remaining_towns) > 5:
        log(f"  ... and {len(remaining_towns)-5} more")
    ...
    for r in all_results:
        ...
        if key:
            seen_keys.add(key)
    log(f"Dedup set: {len(seen_keys)} unique names")
    ...
    try:
        for town in remaining_towns:
            ...
            log(f"\n--- {town_name} ({dist}km, tier={tier}, {len(terms)} terms) ---")
            ...
            for term in terms:
                ...
                try:
                    ...
                    total_new += new_count
                    consecutive_errors = 0
                    log(f"  Found {len(raw_results)} listings, {new_count} new | Total: {len(all_results)}")
            ...
            if consecutive_errors >= 5:
                break
            log(f"  Town '{town_name}' done. Total: {len(all_results)}")
    ...
    log("\n" + "=" * 60)
    log(f"Session complete. Searches: {search_count}, New: {total_new}, Total: {len(all_results)}")
    log("=" * 60)
