#!/usr/bin/env python3
"""
Job Application Scraper v4 - Robust production version.
- Injects JS via add_script_tag to avoid escaping issues
- Handles cookie consent dialogs
- Navigates to job listing pages directly
- Filters for actual job postings (not service categories)
...
PROGRESS_FILE = "/home/z/my-project/job_scraper_v4_progress.json"
LOG_FILE = "/home/z/my-project/job_scraper_v4_log.txt"
...
JS_EXTRACT_FILE = "/home/z/my-project/js_job_extractor.js"

# ── Write the JS extraction function to a file ────────────────────────────────
JS_EXTRACT_CODE = r"""
...
IT_KEYWORDS = [
    ...
    'netzwerk', 'devops', 'cloud', 'data ', 'datenbank', 'cyber', 'security',
    'entwickler', 'developer', 'administrator', 'admin', 'linux', 'sap ',
    'azure', 'aws ', 'frontend', 'backend', 'fullstack', 'full-stack',
    'infrastruktur', 'support', 'consultant', 'engineer', 'digital',
    'webentwickl', 'scrum', 'projektmanag', 'anwendungs', 'application',
    'desktop', 'technology', 'technolog', 'digitalisier', 'comput',
...
SENIOR_KEYWORDS = [
    'senior', 'lead', 'director', 'head of', 'vp ', 'chief', 'principal',
    'staff', 'expert', 'experte',
]
...
def load_companies():
    ...
    for r in range(2, ws1.max_row + 1):
        ...
        if name not in companies:
            companies[name] = {
                "name": name, "kategorie": ws1.cell(r, 5).value or "",
...
class JobScraper:
    ...
    async def inject_js(self):
        """Inject the job extraction JS functions."""
        try:
            # Use evaluate with try/catch instead of add_script_tag
            await self.page.evaluate("""() => {
                if (window.__extractJobs) return;
                window.__extractJobs = function() {
                    const jobWords = ['entwickler', 'developer', 'administrator', 'engineer',
                        'berater', 'consultant', 'trainee', 'junior', 'senior', 'azubi',
                        'praktikant', 'werkstudent', 'ingenieur', 'fachinformatiker',
            ...
            log(f"  ⚠️ JS inject failed: {str(e)[:60]}")

    async def get_career_links(self, base_url):
        """Find career page links using injected JS."""
        try:
            links = await self.page.evaluate(
                'window.__extractLinks(["karriere", "jobs", "stellen", "career", "bewer", "offene", "vacanc", "position", "ausschreib", "unser team", "arbeit bei", "work with", "join", "now hiring", "offene positionen"])'
            )
        except:
            links = []
        ...
        standard = ["/karriere", "/jobs", "/stellenangebote", "/career", "/careers",
                    ...
                    "/offene-stellen", "/bewerbung", "/vacancies",
                    "/offene-positionen", "/stellenangebote.html"]
    ...
    async def extract_jobs_from_page(self, page_url):
        """Extract job listings using injected JS."""
        jobs = []
        ...
        # Inject JS and call extraction
        try:
            await self.inject_js()
            raw_jobs = await self.page.evaluate("window.__extractJobs()")
        except:
            raw_jobs = []
        ...
        for item in raw_jobs:
            ...
            if not is_it_job(title + " " + item.get("text", "")[:200]):
                continue
    ...
    async def find_contact_info(self, base_url):
        ...
        for path in ["/impressum", "/kontakt", "/contact", "/imprint"]:
            try:
                ...
                for em in emails:
                    if not any(g in em.lower() for g in ['noreply', 'no-reply', 'spam', 'example', 'test', 'github']):
                        email = em
                        break
    ...
    async def scrape_company(self, company):
        ...
        for cl in career_links[:8]:
            ...
            # Try IT sub-navigation
            if not page_jobs:
                try:
                    it_links = await self.page.evaluate(
                        'window.__extractLinks(["it", "software", "informatik", "technik", "digital", "technologie", "data"])'
                    )
                    for nav in it_links[:3]:
                        nav_url = nav["href"]
        ...
        # Step 5: Add Arbeitsagentur jobs
...
async def main():
    log("=" * 60)
    log("Job Application Scraper v4 - Starting")
    log("=" * 60)

    # Write JS file
    with open(JS_EXTRACT_FILE, "w", encoding="utf-8") as f:
        f.write(JS_EXTRACT_CODE)
    log(f"JS extractor written to {JS_EXTRACT_FILE}")
    ...
    try:
        ...
        async with async_playwright() as p:
            ...
            for idx, (orig_idx, company) in enumerate(to_process):
                ...
                try:
                    ...
                    log(f"  TB: {traceback.format_exc()[:300]}")
                    jobs, email, ansprech = [], "", ""
                ...
                if processed_count % PUSH_EVERY == 0:
                    ...
                    save_excel(jobs_all)
                    git_push(f"Job Scraper v4: {processed_count} co | {len(jobs_all)} jobs | {datetime.now().strftime('%m-%d %H:%M')}")
        ...
        save_excel(jobs_all)
        git_push(f"Job Scraper v4 DONE: {processed_count} co | {len(jobs_all)} jobs | {datetime.now().strftime('%m-%d')}")
